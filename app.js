...
import { firebaseConfig, ADMIN_EMAILS, PERSONEL_EMAILS, GARAGE_INFO } from "./firebase-config.js";

const GARAGE_NAME = GARAGE_INFO?.name || "Garaj";
const GARAGE_SHORT_NAME = GARAGE_INFO?.shortName || GARAGE_NAME;
const GARAGE_SUBTITLE = GARAGE_INFO?.subtitle || "Profesyonel Araç Servis Takip";
const GARAGE_LOGO = GARAGE_INFO?.logo || "logo.png";
const GARAGE_SLUG = (GARAGE_INFO?.slug || GARAGE_NAME).toLocaleLowerCase("tr-TR").replace(/[^a-z0-9ğüşöçıİĞÜŞÖÇ]+/gi,"-").replace(/^-+|-+$/g,"");
const STORE_KEY = `${GARAGE_SLUG}_data`;
const AUTH_KEY = `${GARAGE_SLUG}_auth`;
const DELETE_PASSWORD = "212198";

const firebaseApp = initializeApp(firebaseConfig);
const auth = getAuth(firebaseApp);
const firestoreDb = getFirestore(firebaseApp);
const SHARED_DATA_DOC = doc(firestoreDb, "garages", GARAGE_SLUG);
let activeUser = null;
...
function applyBranding(){
  document.title = `${GARAGE_NAME} | Araç Takip`;
  document.querySelectorAll('img[src="logo.png"], .brand-img, .login-brand-img, .header-brand-logo').forEach(img => {
    img.src = GARAGE_LOGO;
    img.alt = GARAGE_NAME;
  });
  const loginTitle = document.querySelector('.login-card h1');
  if(loginTitle) loginTitle.textContent = GARAGE_NAME;
  const brandTitle = document.querySelector('.brand h1');
  if(brandTitle) brandTitle.textContent = GARAGE_NAME;
  const brandSub = document.querySelector('.brand p');
  if(brandSub) brandSub.textContent = GARAGE_SUBTITLE;
  const loginEmail = document.getElementById('loginEmail');
  if(loginEmail) loginEmail.placeholder = (ADMIN_EMAILS && ADMIN_EMAILS[0]) || 'admin@firma.com';
  const appleTitle = document.querySelector('meta[name="apple-mobile-web-app-title"]');
  if(appleTitle) appleTitle.setAttribute('content', GARAGE_SHORT_NAME);
}
...
function renderSettings(){
  document.getElementById("settings").innerHTML = `
    ...
    <div class="panel">
      ...
      <pre class="code-box">ADMIN_EMAILS = ["admin@firma.com"]
PERSONEL_EMAILS = ["personel@firma.com"]</pre>
    </div>`;
}
...
function serviceSinglePlainText(serviceId){
  ...
  let text = `${GARAGE_NAME.toLocaleUpperCase("tr-TR")}\n`;
  text += `------------------------------\n\n`;
...
function serviceSingleHtml(serviceId){
  ...
  return `
    ...
    <body>
      ...
      <div class="footer">Bu çıktı ${GARAGE_NAME} sistemi üzerinden oluşturulmuştur.</div>
    </body>
    </html>
  `;
}
...
function customerAccountHtml(customerId){
  ...
  return `
    ...
    <body>
      <button onclick="window.print()" style="padding:10px 14px;margin-bottom:14px">Yazdır / PDF Kaydet</button>
      <div class="head"><div><h1>${GARAGE_NAME}</h1><h2>Müşteri/Firma Hesap Dökümü</h2><p class="muted">Servis, tahsilat ve cari borç raporu</p></div><div><p><b>Çıktı Tarihi:</b> ${today()}</p></div></div>
...
function serviceHistoryPlainText(vehicleId){
  ...
  const title = `${GARAGE_NAME} - Servis Geçmişi`;
  const vehicleName = `${v?.noPlateName ? v.noPlateName + " / " : ""}${v?.plate || "-"}`;
...
window.exportData = function(){
  ...
  a.href = url; a.download = `${GARAGE_SLUG}-yedek.json`; a.click();
